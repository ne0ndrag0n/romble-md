...
u8 BinarySizedArray_takeBits( BinarySizedArray* this, u8 bitsRequested ) {
	...
	if( bitsRequested > bitsRemaining ) {
		...
		desired = *( ( u8* )( sizedArray->items ) ) & ( ( 1 << bitsRemaining ) - 1 );
		...
		SizedArray_burnBytes( sizedArray, 1 );
		...
		desired |= ( *( ( u8* )( sizedArray->items ) ) & ( ( 1 << bitsRequested ) - 1 ) ) << bitsRemaining;
		...
		desired = *( ( u8* )( sizedArray->items ) ) & ( ( 1 << bitsRequested ) - 1 );
		this->bitsUsed += bitsRequested;
	}
